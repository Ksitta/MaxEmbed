...
def _fill_missing_features_and_split(chunk, series_list_dict):
    for cid, col in enumerate(chunk.columns):
        NAN_VALUE = INT_NAN_VALUE if col in dense_cols else CAT_NAN_VALUE
        result_series = chunk[col].fillna(NAN_VALUE)
        series_list_dict[col].append(result_series)


def PrintColumn(col, needed_col, series, debug=""):
    if col == needed_col:
        print(debug, series)
...
def _merge_and_transform_series(src_series_list, col, dense_cols,
    ...
    # ERASE_COUNT = 6
    ERASE_COUNT = 0
    if ERASE_COUNT != 0 and col != 'label':
        unique_value_counts = result_series.value_counts()
        unique_value_counts = unique_value_counts.loc[unique_value_counts >= ERASE_COUNT]
    ...
    if col == 'label' or col in dense_cols:
        ...
        unique_value_counts = result_series.value_counts()
        result_series = result_series.apply(lambda x:
                                            unique_value_counts.index.get_loc(
                                                x)
                                            )
    ...
    min_max = (np.int64(result_series[col].min()),
               np.int64(result_series[col].max()))
    if col != 'label':
        logging.info('column {} [{}, {}]'.format(
            col, str(min_max[0]), str(min_max[1])))
...
def _merge_columns_and_feature_cross(series_list, min_max, feature_pairs,
    ...
    if feature_cross != 0:
        for idx, pair in enumerate(feature_pairs):
            ...
            df.insert(NUM_INTEGER_COLUMNS + 1 + idx,
                      crossed_column, crossed_column_series)
            ...
            logging.info('column {} offset {}'.format(
                crossed_column, str(offset)))
...
def preprocess(src_txt_name, dst_txt_name, normalize_dense, feature_cross, drop_cnt):
    ...
    df.drop(df.columns[:drop_cnt], axis=1, inplace=True)
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(
        description='Preprocssing Avazu Dataset')
    ...
    arg_parser.add_argument('--feature_cross', type=int, default=0)
    arg_parser.add_argument('--drop_cnt', type=int, default=0)
    ...
    del df['id']
    # del df['hour']
    df.to_csv(src_csv_path+'.tmp', index=False, header=False)
    preprocess(src_csv_path+'.tmp', dst_csv_path,
               normalize_dense, feature_cross, args.drop_cnt)
